...
def log_max_normalize(logprobs: dict[T, float]) -> dict[T, float]:
    ...
    normalizer = float(max(float(v) for v in logprobs.values()))
    return {k: v - normalizer for k, v in logprobs.items()}
...
def remove_logit_bias(
    ...
    missing_key = object()
    values = list(biased_logprobs.values())
    log_sum = scipy.special.logsumexp(values)
    missing_prob = 1.0 - np.exp(scipy.special.logsumexp(list(biased_logprobs.values())))  # type: ignore
